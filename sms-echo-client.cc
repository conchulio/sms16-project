...
void 
SmsEchoClient::SetRemote (Ipv6Address ip, uint16_t port)
{
  NS_LOG_FUNCTION (this << ip << port);
  m_peerAddress = Address (ip);
  m_peerPort = port;
  ...
  if (m_dataSize)
    {
      ...
      //
      NS_ASSERT_MSG (m_dataSize == m_size, "SmsEchoClient::Send(): m_size and m_dataSize inconsistent");
      NS_ASSERT_MSG (m_data, "SmsEchoClient::Send(): m_dataSize but no m_data");
